...
Mesh::Mesh(Real_t _Area) noexcept {

	Real_t Boundary[8] = {0., 0., 1., 0., 1., 1., 0., 1.};
	Real_t PML[8] = {0., 0., 1., 0., 1., 1., 0., 1.};
	construct(Boundary, 4, PML, 4, _Area);

}


Mesh::Mesh(Real_t* _Boundary, int32_t _nBoundary, Real_t* _PML, int32_t _nPML, Real_t _Area) noexcept{
	construct(_Boundary, _nBoundary, _PML, _nPML, _Area);
}


void Mesh::construct(Real_t* _Boundary, int32_t _nBoundary, Real_t* _PML, int32_t _nPML, Real_t _Area) noexcept{

	min_area            = _Area;
	auto _BoundaryPtr   = _Boundary;
	auto _BoundarySize  = _nBoundary;
...
void Mesh::Promote(int32_t _deg) noexcept{
	...
	auto deg = _deg;
	if (deg <= 0) {printf("Degree has to be positive. Please use first order instead.\n");}
	if (deg >= 20) {printf("Degree Too Large\n");}
	else{
		topology.clear();
